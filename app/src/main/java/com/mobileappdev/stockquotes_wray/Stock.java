...
public class Stock implements Serializable
{
    ...
    public void load() throws MalformedURLException, IOException
    {
        ...
        if (DEBUG)
            Log.i(TAG_PREFIX + "S.L()", "url = " + url);

        URLConnection connection = url.openConnection();

        if (DEBUG)
            Log.i(TAG_PREFIX + "S.L()", "url connection opened");

        InputStreamReader isr = new InputStreamReader((connection.getInputStream()));

        if (DEBUG)
            Log.i(TAG_PREFIX + "S.L()", "input stream reader created");

        BufferedReader in = new
                BufferedReader(isr);

        if (DEBUG)
            Log.i(TAG_PREFIX + "S.L()", "buffered reader created");
        ...
        if (line != null && line.length() > 0)
        {
            ...
            if (DEBUG)
                Log.i(TAG_PREFIX + "S.L()", "name = " + name);
